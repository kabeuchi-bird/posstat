...
日本語コーパス(.txt)の品詞・カナ・文節統計を集計し、自己完結型の HTML レポートと JSON を出力するクロスプラットフォーム CLI スクリプトです。
...
読みの長さで、打鍵数に対応します。英字は表層だと実態より長く出るため
(`programming` = 11 文字に対し読みは `プログラミング` = 7)、カナ文字数側を併記します。
...
表はクリックでソート、テキストフィルタ付き。「絶対来ない」ペアの表は先行カナ・
後続カナそれぞれの絞り込み欄も持ち、行全体フィルタと AND で効きます。
...
セクション 11 は大岡俊彦氏の「繋ぎの語」抽出ルール(deprel: case/mark/aux/cop/cc/discourse/fixed、
指示代名詞、形式名詞、接続副詞、補助動詞)で「繋ぎの語」を判定し、連続する繋ぎ語が
膠着した全体を 1 塊(チャンク)として、繋ぎ/内容それぞれのチャンク内カナ 2-gram・3-gram
とチャンク境界跨ぎの連接を集計します。3-gram 表および繋ぎチャンク頻度表は
上位 3000 件のみ HTML に掲載し、全量は stats.json に出力します。
...
- `forbidden_pairs` の `pmi` は `log2( P(先行と後続が隣接) / (P(先行) × P(後続)) )`。
  独立を仮定した期待に対し実際の共起が何倍かを表し、負なら並びにくい
  (-1 で期待の 1/2、-3 で 1/8)。観測ゼロのときは `null`(Rust 側は `Option<f64>`)。
  `expected` は `P(先行) × P(後続) × 隣接ペア総数` で、`min_count` 未満の組合せは
  標本が少なく偶然ゼロになりうるため判定対象から除外している
...
- 二重鉤括弧 `『』` は GiNZA の文節境界認識を乱し、述語まで巻き込んだ長大な 1 文節に
  なることがあります(品詞・係り受けは `「」` と同一なのに文節分割だけが変わる)。
  このため Stage 2 に渡す入力でのみ `『』` → `「」` に正規化します。原文は変更しないので
  Stage 1 の記号前後統計は影響を受けず、カナ統計も括弧は PUNCT として除外済みです
  (`--dump-tsunagi-text` と最長文節の診断表示だけ `「」` になります)。
